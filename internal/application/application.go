...
func (a ApplicationService) GetApplications() ([]models.Application, error) {
	...
	err := a.db.Select(&applications, "SELECT * FROM application")
	if err != nil {
		return nil, err
	}
...
func (a ApplicationService) GetApplicationByID(id string) (models.Application, error) {
	...
	err := a.db.Get(&application, "SELECT * FROM application WHERE id=$1", id)
	if err != nil {
...
func (a ApplicationService) CreateApplication(application models.Application) error {
	_, err := a.db.NamedExec("INSERT INTO application (id, client_id, client_secret, redirect_uri, grant_types) VALUES (:id, :client_id, :client_secret, :redirect_uri, :grant_types)", application)
	if err != nil {
		return err
	}
	return nil
...
func (a ApplicationService) ValidateClientId(clientId string) (bool, error) {
	var count int
	err := a.db.Get(&count, "SELECT COUNT(*) FROM application WHERE client_id=$1", clientId)
	return count == 1, err
}

func (a ApplicationService) ValidateClientSecret(clientId, clientSecret string) (bool, error) {
	var count int
	err := a.db.Get(&count, "SELECT COUNT(*) FROM application WHERE client_id=$1 AND client_secret=$2", clientId, clientSecret)
	return count == 1, err
}

func (a ApplicationService) ValidateRedirectUri(clientId, redirectUri string) (bool, error) {
	var count int
	err := a.db.Get(&count, "SELECT COUNT(*) FROM application WHERE client_id=$1 AND redirect_uri=$2", clientId, redirectUri)
	return count == 1, err
}
